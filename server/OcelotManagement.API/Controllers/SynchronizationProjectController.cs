...
namespace OcelotManagement.API.Controllers
{
    ...
    [Authorize]
    public class SynchronizationProjectController : ControllerBase
    {
        private const int TYPE = 1;
        private readonly ProjectService _projectService;

        public SynchronizationProjectController(ProjectService projectService)
        {
        ...
        public async Task<ActionResult<ApiResult<ApiPagedResult<ProjDto>>>> GetAll([FromQuery] ApiPageArg arg)
        {
            ...
            var result = ApiResult<ApiPagedResult<ProjDto>>.SuccessResult(projects, "获取同步项目列表成功");
            return Ok(result);
        }
